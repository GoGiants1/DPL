...
class StableDiffusion_MyPipeline(DiffusionPipeline):
    ...
    # @staticmethod
    # def _compute_entropy(attention_maps: torch.Tensor,indices_to_alter: List[int],):
    #     _entropy = 0
    #     for i in indices_to_alter:
    #         attn = attention_maps[:, :, i].view(-1)
    #         attn = torch.nn.functional.softmax(attn, dim=0)
    #         _entropy += entropy(attn)
    #     return _entropy/float(len(indices_to_alter))
    ...
    # @staticmethod    
    # def _compute_difference(attention_maps: torch.Tensor,indices_to_alter: List[int],):
    #     cat_attn = attention_maps[:,:,indices_to_alter[0]]
    #     dog_attn = attention_maps[:,:,indices_to_alter[1]]

    #     difference = (cat_attn/cat_attn.max() - dog_attn/dog_attn.max()).abs().mean()
    ...
    def _aggregate_and_get_max_attention_per_token(
        ...
        # else:
        #     max_attention_per_index = self._compute_max_attention_per_index_nosoftmax(
        #         attention_maps=attention_maps,
        #         indices=indices,
        #         smooth_op = smooth_op,
            # )
    ...
    ### NOTE: new version of computing max attention
    # @staticmethod
    # def _compute_max_attention_per_index_nosoftmax(
    #     attention_maps: torch.Tensor,
    #     indices: List[int],
    #     smooth_op = True,
    # ) -> List[torch.Tensor]:
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                ### NOTE: BG alpha 0.9 T=0; beta 0.1 T=49
                # max_target_loss = alpha_max - (alpha_max - beta_max)/50*i
                # BG_target_loss = alpha_ent - (alpha_ent - beta_ent)/50*i
                # cos_target_loss = alpha_cos - (alpha_cos - beta_cos)/50*i
                ...
                if attn_inner_steps > 1:
                    with torch.enable_grad():
                        ...
                        for j in range(attn_inner_steps):
                            ...
                            ### NOTE: later import the entropy and cosine loss
                            if lam_maxattn>0:
                            ...
                            # NOTE: update once even with smaller loss
                            ...
                            ### NOTE: this part decides if we want to update when loss is smaller than the target loss
                ...
                ### NOTE: comment the next line for now since we don't need it.
